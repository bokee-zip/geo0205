...
const SlideContent: React.FC<{ slide: SlideData }> = ({ slide }) => {
  ...
  const closeModal = () => setSelectedDeliverable(null);
    ...
    case 'image-grid':
      return (
        <div className="h-full flex flex-col space-y-8">
          ...
          <div className={`flex-1 grid gap-6 ${slide.images?.length === 2 ? 'grid-cols-2' : 'grid-cols-2 lg:grid-cols-4'}`}>
            {slide.images?.map((img, idx) => (
              <div key={idx} className="flex flex-col bg-white rounded-3xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl transition-all group">
                <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden">
                  <img src={img} alt={`Capture ${idx} `} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute inset-0 bg-slate-900/10 group-hover:bg-transparent transition-colors" />
                  <div className="absolute top-4 left-4">
                    <span className="bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full text-[20px] font-black text-[#0055FF] shadow-sm uppercase tracking-widest">
                      CAP 0{idx + 1}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  {slide.items?.[idx] && (
                    <div className="space-y-2">
                      <h4 className="text-[24px] font-black text-[#111]">{slide.items[idx].label}</h4>
                      <p className="text-[20px] font-bold text-slate-400">{slide.items[idx].value}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="flex-1 grid grid-cols-4 gap-4 items-stretch py-2">
            {slide.items?.map((step, idx) => (
              <div key={idx} className={`relative flex flex-col px-6 py-6 rounded-[2.5rem] border transition-all duration-300 ${idx === 1 ? 'bg-rose-50 border-rose-200 shadow-xl shadow-rose-100/50' : 'bg-white border-slate-100 shadow-sm hover:shadow-md'}`}>
                ...
                <div className="w-full h-48 bg-slate-100 rounded-3xl mb-6 flex items-center justify-center border border-slate-200 overflow-hidden relative group flex-shrink-0">
